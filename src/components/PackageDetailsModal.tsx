...
export default function PackageDetailsModal({ packageIndex, onClose }: Props) {
  ...
  const agentTip = pkg.agentTip;

  return (
    <div
      className='fixed inset-0 z-[60] flex items-center justify-center lg:p-4'
      onClick={onClose}
    >
      {/* Backdrop */}
      ...
      {/* Modal */}
      <div
        className='relative w-full h-full lg:h-auto lg:max-h-[85vh] lg:max-w-2xl flex flex-col bg-dark-800 rounded-none lg:rounded-2xl border-0 lg:border border-white/10 shadow-2xl animate-[fadeInUp_0.3s_ease] overflow-hidden'
      ...
      >
        ...
        <div className='flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar'>
          ...
          <div>
            ...
            <div className='space-y-3'>
              {includes.map((item, i) => (
                <div
                  key={i}
                  className='p-3 bg-white/5 rounded-xl border border-white/10 flex gap-3'
                >
                  <div className='w-5 h-5 rounded-full bg-green-500/20 flex items-center justify-center shrink-0 mt-0.5'>
                    <FiCheck className='w-3 text-green-500' />
                  </div>
                  <div>
                    <h5 className='text-sm font-semibold text-white/90 leading-tight'>
                      {item.title}
                    </h5>
          ...
          <div className='p-4 bg-gold-400/5 rounded-xl border border-gold-400/20'>
            <p className='text-xs text-gold-400/90 font-medium mb-2'>
              {t('notesTitle')}
            </p>
